...
class VipTransactionInsertListener
{
    ...
    public function handle(TransactionInsertEvent $event)
    {
        ...
        if ($transaction['amount'] >= 0) {
            if (config('playbetr.vip.debug')) {
                \Log::debug(__FUNCTION__.' VIP Not a debit');
            }
            return false;
        }
        ...
        if (!isset($config['propertySettings'][$transaction['property_id']])) {
            if (config('playbetr.vip.debug')) {
                \Log::debug(__FUNCTION__.' VIP No config property_id');
            }
            return false;
        }
        ...
        if ($settings['default_bank_id'] != $transaction['bank_id']) {
            if (config('playbetr.vip.debug')) {
                \Log::debug(__FUNCTION__.' VIP Bank ID does not match');
            }
            return false;
        }
        ...
        if (!$source_group_id) {
            if (config('playbetr.vip.debug')) {
                \Log::debug(__FUNCTION__.' VIP No source group ID');
            }
            return false;
        }
        ...
        if (!isset($settings['handlers'][$source_group_id])) {
            if (config('playbetr.vip.debug')) {
                \Log::debug(__FUNCTION__.' VIP No handler for source group id');
            }
            return false;
        }
